...
public class SelectionMust {
    ...
    public void beCreatedCorrectly_whenInformationIsCorrect() {
        Selection sut = new Selection(SELECTION_ID, SELECTION_PRODUCT, SELECTION_QUANTITY, SELECTION_DISCOUNT, SELECTION_TOTAL);
    ...
    public void throwException_whenIdIsInvalid(@FromDataPoints("invalid ids") long invalidId) {
        Exception exception = assertThrows(RuntimeException.class, () -> new Selection(invalidId, SELECTION_PRODUCT, SELECTION_QUANTITY, SELECTION_DISCOUNT, SELECTION_TOTAL));
        assertEquals("El id es inválido", exception.getMessage());
    }

    @Test
    public void throwException_whenProductIsInvalid() {
        Exception exception = assertThrows(RuntimeException.class, () -> new Selection(SELECTION_ID, null, SELECTION_QUANTITY, SELECTION_DISCOUNT, SELECTION_TOTAL));
    ...
    public void throwException_whenQuantityIsInvalid(@FromDataPoints("invalid ids") int invalidQuantity) {
        Exception exception = assertThrows(RuntimeException.class, () -> new Selection(SELECTION_ID, SELECTION_PRODUCT, invalidQuantity, SELECTION_DISCOUNT, SELECTION_TOTAL));
        assertEquals("La cantidad es inválida", exception.getMessage());
    }

    @Test
    public void throwException_whenDiscountIsInvalid() {
        Exception exception = assertThrows(RuntimeException.class, () -> new Selection(SELECTION_ID, SELECTION_PRODUCT, SELECTION_QUANTITY, -1, SELECTION_TOTAL));
    ...
    public void throwException_whenTotalIsInvalid(@FromDataPoints("invalid totals") int invalidTotal) {
        Exception exception = assertThrows(RuntimeException.class, () -> new Selection(SELECTION_ID, SELECTION_PRODUCT, SELECTION_QUANTITY, SELECTION_DISCOUNT, invalidTotal));
